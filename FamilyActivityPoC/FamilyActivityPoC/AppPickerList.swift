...
struct AppPickerList: View {
    ...
    private func rowView(_ row: Row) -> some View {
        ...
        return Button { toggle(row.name) } label: {
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
                    .foregroundStyle(isLocked ? Color.secondary : Color.accentColor)
                    // 勾选与否**只有这一个视觉信号**（颜色之外还有形状，这点是对的），
                    // 但 SF Symbol 的名字对 VoiceOver 没有意义 —— 读出来是
                    // "checkmark dot circle dot fill"。所以：图标当装饰藏掉，
                    // 状态改用下面那个 `.isSelected` trait 告诉它。
    ...
    /// 勾选 / 取消勾选。
    ///
    /// 从按钮闭包里抽出来：这是本视图**唯一的分支逻辑**。留在闭包里的话，
    /// 读的人要在两层布局中间做一次状态推理 —— 而它跟布局毫无关系。
