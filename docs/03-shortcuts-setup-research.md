...
> **状态：调研已完成，路线 A 已实施并在真机验证（见第 4 节）。**
> 本文只记录"什么是真的"，以及两条候选路线。
>
...
**已于 iPhone 15 Pro / iOS 26.7 上实测通过。** 下面是当初要验的实验，保留原始设计。
...
| 待验的事 | 结果 |
| --- | --- |
| 触发器能否一次勾选多个 App | ✅ 能（2.1 的二进制结论在真机复现） |
| 「获取当前 App」是否存在于动作列表 | ✅ 存在（用户系统 26.7 > 18.2） |
| 多选触发器下报的名字对不对 | ✅ **各报各的，都对** —— 分别打开两个 App，通知分别报出对应的名字 |
| 触发器自带的属性里有没有 Bundle Identifier | ⏳ **没有验**，也不需要了 —— A 既然成立，就没有理由去动它 |

三点补充，都是实施之后才看清的：

1. **这条要验的实验只勾了 2 个 App；实际方案要勾全部。** 触发器的多选上限、
   以及"勾 100+ 个 App 之后还报不报得对"没有单独验证过。**这是当前最大的开放项**
   （见 README 已知限制与下节）。
2. **App 触发器支持多选 ≠ 有"全选"按钮。** 选择器里能不能一次全选，没有确认过；
   没有的话"全选"就是上百次点击，退路是"勾选你关心的十几个 App"，
   那会让"新增 App 不用再碰 Shortcuts"这条部分失效。
3. 报上来的是**本地化显示名**（2.3），不是 bundle id —— 所以它仍会随**系统语言**和
   **App 改名**变化。v2 的对策是"名字只能从系统上报过的列表里选"，
   把拼写风险消掉，但没有消除名字本身会变这件事。
...
> **注（v2 实施后追记）：这一条在路线 A 下没有采用，不要再加回来。**
> 当时的前提是"用户手打 App 名"，所以需要一个原样字符串当键、一个好看的当展示名。
> 路线 A 下名字**由系统上报**、且只能从检测列表里选，键和展示名天生是同一个东西 ——
> 再存两份只会多出一处真相。实际采用的是：`TodoBinding` 存一组名字，
> 匹配时统一走 `TodoBinding.normalize`（trim + 小写）。
