...
| 接线指引 sheet | 五步 ＋ ⚠️「运行前询问」必须关掉 ＋ 排查清单标题。⚠️ **排查清单的正文在屏幕外，没有截图确认** |
...
## 2i. v3 证据：待办页布局改造（2026-09-24）

改动目标（用户原话）：待办一多就要一直上滑，因为「待办列表」和「新增待办」挤在同一页；
列表顺序也要调 —— **待办中在前，已完成在后**。

改完的形态：新增搬进右上角 ＋ 打开的独立 sheet；列表分两段，**已完成默认折叠**；
顶部「接线状态」「通知权限」按用户要求**保持不动**（原话："接线指引保留在顶部，这一块是蛮重要的，是引导用户用的"）。

### 贯穿全程的约束：分组只能是**展示投影**

`TodoStore.bindings` 的数组顺序 = 存储顺序，而它同时是三处**取证输出**的顺序：
`dumpState()` 的 BINDINGS 段、`TodoReminder.snapshot()` 的 `before=/after=`、
`claimTest()` 的逐条快照。在存储上按 `isDone` 重排，那些文本会变 ——
于是"改了个列表样式"会表现成"不变量测试坏了"。所以新增的是两个**只读计算属性**
（`pendingBindings` / `completedBindings`，用 `filter` 做稳定划分），`bindings` 一个字节没动。

**证据 1（逐字节对比，完全不经过 UI）**：`--poc-claim-test` 的脚本恰好包含
「add 两条 → 把 B 标记完成 → 恢复」，正好覆盖 `isDone` 变化的时刻。改动前先跑一次、存下
`poc-state.txt` 的 `CLAIM TEST` 整段，改动后跑同样的命令 —— **diff 为空**。

**证据 2（交错的文件必须按文件顺序打印）**：手工写一个顺序交错的 `todo-bindings.json`
（待办中A, 已完成B, 待办中C, 已完成D）后 `--poc-dump-state`：

```
BINDINGS: 4 (pending=2 done=2)
  - ... apps=[微信]   isDone=false todo="待办A-待办中"
  - ... apps=[QQ]     isDone=true  todo="待办B-已完成"
  - ... apps=[支付宝] isDone=false todo="待办C-待办中"
  - ... apps=[钉钉]   isDone=true  todo="待办D-已完成"
```

打印的是**文件顺序**（A, B, C, D）而不是分组顺序（A, C, B, D）—— 分组确实只活在展示层。
括号里的 `pending=2 done=2` 是本版新加的 summary，让"投影是对 bindings 的划分"进入取证链。

**证据 3（就地翻转）**：`--poc-action-test done 微信` 的 trace：

```
apply action=TODO_ACTION_DONE bindingID=nil app="微信" resolvedBy=name before=微信:pending,QQ:done,支付宝:pending,钉钉:done
apply finished action=TODO_ACTION_DONE after=微信:done,QQ:done,支付宝:pending,钉钉:done
```

微信**原地**翻转成 done，没有跳到数组末尾 —— 若分组渗进存储，它会出现在最后。

### 界面截图

| 截图 | 确认了什么 |
| --- | --- |
| 待办页 | 工具栏 `?` 与 `＋` 在**同一个**玻璃胶囊里共存、＋ 在最右；接线指引与通知权限仍在最上；待办中（3）三条（含多 App 芯片换行、含橙色「未选择 App（不会提醒）」）；已完成（2）收成一行带 chevron，**看不到已完成条目** |
| 待办中为空 | 待办中（0）显示占位行「没有待办中的条目。」而不是光秃秃一个 header；已完成段展开后两条带删除线灰化 |
| ＋ 新增 sheet | 取消 / 新增待办 / 添加（未满足条件时是灰的）；待办内容输入框；选择 App 行；已选芯片 |
| ＋ sheet ＋ 键盘 | **键盘正上方的「完成」渲染出来了** —— `.toolbar(placement: .keyboard)` 在 sheet 里有效；且 `取消`/`添加` 在导航栏里、**键盘盖不住它们** |
| push 选择器 | 返回箭头 ＋ 单个「完成」（根页的「添加」正确让位）；已占用项置灰并标出占用者；footer 里新补了"列表里没有你要的 App？先去打开它一次" |

⚠️ 这些截图**同样是靠临时启动参数**弹出来的（模拟器没有触摸注入），参数已删除并 grep 到零
（`POCUITemp` / `--poc-open-new-todo` / `TEMPORARY` 均 0 命中）。

**一个被截图推翻的假设**：键盘那条 `placement: .keyboard` 工具栏在 sheet 里能否渲染，
是本版唯一没底的点 —— 它一旦不渲染，失败模式正是 2h 补记第 2 条那个"键盘收不起来、没有退路"原地复活。
截图证明它**渲染**。所以输入框行内那个兜底「收起键盘」按钮成了冗余 —— 保留它是**有意的保险**
（真机的键盘高度与附件行为仍需确认），不是必需品；嫌挤可以删。

### 顺手修掉的两个既有缺陷

1. **`canAdd` 与 `TodoStore.add` 的判据不一致**：界面用 `.whitespaces`（**不含**换行符），
   store 用 `.whitespacesAndNewlines`。所以输入一个**纯换行**（中文输入法下先换行再打字很常见）
   会通过界面判据 → store 裁空后返回 nil → **sheet 关掉、什么都没加、也没有任何提示**。
   新 sheet 改用 `.whitespacesAndNewlines`，两边对齐。
2. **`AppPickerSheet` 里那句「去『接线指引』按四步做一遍」**早该是五步（v2 已改成五步），
   注释里的"四步"也一并改了。另外"列表里没有你要的 App"这个提示此前**只在列表完全空时**才说 ——
   而这正是用户真机撞到的那类困惑，现已补进**非空**列表的 footer。

### 本次**没有**验证的

- 点 ＋ 打开 sheet、「取消」/ 下滑关闭 / 「添加」提交 —— 都需要真实触摸。
- **在 sheet 里输入多行后能否收起键盘**（即 2h 补记第 2 条在新位置上是否仍修好）——
  截图只证明键盘附件**渲染**，不证明它能被**点**。
- 「选择 App」push / 返回 / 选择器「完成」出栈后选择是否保留。
- 「已完成（N）」**点开与收起**（截图只证明两种状态各自渲染正确，切换要靠触摸）。
- 展开后已完成行的左滑「恢复待办」/ 右滑「删除」。
- 两个工具栏按钮在真机真字号下是否好点；最大动态字体下会不会被收进溢出菜单。
- 折叠状态下**无法删除**已完成条目（没有渲染的行就没有 swipeActions）—— 行为已知，观感未验。
- **v3 整体在真机上没跑过。**
