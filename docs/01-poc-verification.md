...
- **提醒频率的体验问题（待产品决策）**：当前规则是"待办未完成 → 每次打开目标 App 都提醒"，
  实测中很快堆积了 16 条通知。这是按既定规则实现的（「稍后再说」的语义就是"下次还提醒"），
  但实际观感可能偏吵。可选调整方向见第 5 节。
...
1. **提醒频率**：现在"待办未完成 → 每次打开目标 App 都提醒"，实测很快堆了 16 条通知。
   可选方向：
   - 保持现状（「稍后再说」的语义就是"下次还提醒"）
   - 改为"同一待办每天只提醒一次"
   - 改为"同一待办只在首次打开时提醒一次，之后静默直到用户主动查看"
2. **按钮要下拉横幅才能看到**（见 2g）。这是平台约束，绕不开。若觉得这一步摩擦太大，
   替代形态是：通知只负责提醒，用户点击后进入 App 内的待办卡片再操作。
3. **提醒文案**：当前为「打开 微信 时想起」，待确认是否合适。
